...
import { EVENT_CARD_EDITOR_NAME } from './const';
import { css, html, LitElement, nothing } from 'lit';
import { customElement, property, state } from 'lit/decorators.js';
import { entityCardConfigStruct } from './event-card-config';
import { getPatternOthersSchema, getPatternSchema, getSchema } from './formSchemas';
import { fireEvent } from '../../utils/fireEvent';

import './event-card-pattern-editor';

import type { EventCardConfig } from './event-card-config';
import type { CSSResultGroup, PropertyValues } from 'lit';
import type { HomeAssistant } from '../../utils/ha';
import type { SubElementEditorConfig } from './event-card-pattern-editor';
...
@customElement(EVENT_CARD_EDITOR_NAME)
class EventCardEditor extends LitElement {
  ...
  public setConfig (config: Partial<EventCardConfig>): void {
  ...
  private renderFormPatternsEditor () {
    ...
    if (this.subElementEditorConfig) {
      ...
      return html`
        <div class="header" id="eventcard-pattern-editor">
  ...
  public static get styles (): CSSResultGroup {
    return [
      css`
        #eventcard-pattern-editor header {
          display: flex;
          justify-content: space-between;
          align-items: center;
        }

        #eventcard-pattern-editor .back-title {
            display: flex;
            align-items: center;
            font-size: 18px;
        }

        #eventcard-pattern-editor ha-icon {
             display: flex;
             align-items: center;
